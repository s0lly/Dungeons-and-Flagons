...
void Graphics::DrawSpriteScaledUp(int x, int y, const Surface & s, int scale, Color chroma)
{
	DrawSpriteScaledUp(x, y, s.GetRect(), s, scale, chroma);
}

void Graphics::DrawSpriteScaledUp(int x, int y, const RectI & srcRect, const Surface & s, int scale, Color chroma)
{
	DrawSpriteScaledUp(x, y, srcRect, GetScreenRect(), s, scale, chroma);
}

void Graphics::DrawSpriteScaledUp(int x, int y, RectI srcRect, const RectI & clip, const Surface & s, int scale, Color chroma)
{
	assert(srcRect.left >= 0);
	assert(srcRect.right <= s.GetWidth());
	assert(srcRect.top >= 0);
					...
					for (int scaleRow = 0; scaleRow < scale; scaleRow++)
					{
						for (int scaleCol = 0; scaleCol < scale; scaleCol++)
						{
								...
								&& y + (sy - srcRect.top) * scale + scaleRow >= clip.top && y + (sy - srcRect.top) * scale + scaleRow < clip.bottom)
							{
								...
								//x + sx * scale - srcRect.left * scale + scaleCol
								//y + sy * scale - srcRect.top + scaleRow
							}
						}
					}
				}
...
void Graphics::DrawSpriteScaledUpRotated(int x, int y, RectI srcRect, const RectI & clip, const Surface & s, int scale, MOVETYPE direction, Color chroma)
{
	if (x + srcRect.GetWidth() * scale >= clip.left && x < clip.right
		&& y + srcRect.GetHeight() * scale >= clip.top && y < clip.bottom)
	{
